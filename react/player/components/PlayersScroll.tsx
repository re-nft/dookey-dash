...
export const PlayersScroll = React.memo(
  function PlayersScroll({
    ...
    React.useEffect(() => {
      ...
      setState(s => {
        ...
        const nextPlayerIds = maybeNextPlayers.map(({address}: Player) => address);

        const nextPlayers = maybeNextPlayers
          .filter(({address}, i) => nextPlayerIds.indexOf(address) === i);
